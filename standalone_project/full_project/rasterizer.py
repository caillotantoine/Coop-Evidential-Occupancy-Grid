...
def projector (len: c_int, labels: np.ctypeslib.ndpointer(dtype=np.int32), fp_vec:np.ctypeslib.ndpointer(dtype=np.float32), map_out:np.ctypeslib.ndpointer(dtype=np.uint8), mapsize: c_float, gridsize:c_int) -> None:
    rasterizer.projector(len, labels, fp_vec, map_out, mapsize, gridsize)
