...
class BBoxManager:
    ...
    def draw2DBoxes(self, image, bboxes):
        # take as input the image and the list of bounding box to draw
        # apply the draw to the image
        ...
        for bbox in bboxes:
            ...
            bbox_vert_world = self.placeBBoxInWorld(bbox) # place the 8 corner of the box in the world
            bbox_vert2D_cam = self.compute2Dbbox(image, bbox_vert_world)

            if len(bbox_vert2D_cam) > 0:
                setOfBbox.append(bbox_vert2D_cam)
        ...
        return (self.cv_bridge.cv2_to_imgmsg(cv_img, "bgr8"), setOfBbox) # convert the openCV image to a ROS Image message
